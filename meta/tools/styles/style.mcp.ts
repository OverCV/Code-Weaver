...
async function loadContentOrDefault(filePath: string, defaultContent: string): Promise<string> {
    try {
        ...
        debug(`Error leyendo archivo ${filePath}: ${err}`)
        return defaultContent
    }
}
...
export function registerStyleMcp(server: McpServer) {
    ...
    server.tool(
        ...
        async ({ modo = "senior", proy: proyecto = "general", extra }) => {
            try {
...
- Implementar una tarea específica`
                ...
                debug(`Error en mcd: ${err.message}`)
                return {
                    content: [
                        {
                            type: "text",
                            text: `Error inicializando framework: ${err.message}`
                        }
                    ]
                }
            }
    ...
    server.tool(
        ...
        async () => {
            try {
                ...
                debug(`Error en listar-perfiles: ${err.message}`)
                return {
                    content: [
                        {
                            type: "text",
                            text: `Error listando perfiles: ${err.message}`
                        }
                    ]
                }
            }
    ...
    server.tool(
        ...
        async () => {
            try {
                ...
                debug(`Error en listar-proyectos: ${err.message}`)
                return {
                    content: [
                        {
                            type: "text",
                            text: `Error listando tipos de proyecto: ${err.message}`
                        }
                    ]
                }
            }
    ...
    server.tool(
        ...
        async ({ nombre, contenido, descripcion }) => {
            try {
                // Crear directorios si no existen
                await ensureDirectories()
                ...
                debug(`Error en crear-perfil: ${err.message}`)
                return {
                    content: [
                        {
                            type: "text",
                            text: `Error creando perfil: ${err.message}`
                        }
                    ]
                }
            }
    ...
    server.tool(
        ...
        async ({ nombre, contenido, descripcion }) => {
            try {
                ...
                debug(`Error en crear-proyecto: ${err.message}`)
                return {
                    content: [
                        {
                            type: "text",
                            text: `Error creando tipo de proyecto: ${err.message}`
                        }
                    ]
                }
            }
